...
static MESSAGES: Lazy<HashMap<&'static str, &'static str>> = Lazy::new(|| {
    ...
    #[cfg(feature = "pdf")]
    m.insert(
        "help.extract.supported_files",
        "지원 파일: .docx (Word), .pptx (PowerPoint), .pdf, .xlsx (Excel)",
    );
    #[cfg(not(feature = "pdf"))]
    m.insert(
        "help.extract.supported_files",
        "지원 파일: .docx (Word), .pptx (PowerPoint), .xlsx (Excel)",
    );
    m.insert(
        "help.extract.example1",
    ...
    m.insert(
        "help.replace.exclude_pattern",
        "제외 패턴 예시: --exclude \"*.tmp\" --exclude \"backup/*\"",
    );

    // === Create 명령어 상세 도움말 ===
    m.insert("help.create.markdown_features", "지원 Markdown 기능:\n• 제목 (# ## ###)\n• 목록 (-, *, 1.)\n• 표\n• 코드 블록\n• 이미지\n• 링크");
    m.insert(
        "help.create.example1",
        "# Markdown에서 Word 문서 생성\ndox create -f README.md -o output.docx",
    );
    m.insert(
        "help.create.example2",
    ...
    m.insert(
        "help.generate.example1",
        "# 블로그 포스트 생성\ndox generate -p \"Rust 프로그래밍 입문\" -t blog",
    );
    m.insert(
        "help.generate.example2",
