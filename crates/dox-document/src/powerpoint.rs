...
use crate::utils::{copy_zip_with_replacements, extract_text_from_xml, replace_text_in_xml, read_zip_file};
...
impl PowerPointProvider {
    ...
    pub fn open(path: &Path) -> Result<Self, DocumentError> {
        ...
        info!("Loaded PowerPoint document with {} slides", slide_contents.len());
    ...
    fn extract_slide_contents(archive_data: &[u8]) -> Result<Vec<(String, Vec<u8>)>, DocumentError> {
    ...
    pub fn get_slide_text(&self, slide_index: usize) -> Result<String, DocumentError> {
        if slide_index >= self.slide_contents.len() {
            return Err(DocumentError::InvalidStructure {
                reason: format!("Slide index {} out of range (0-{})", slide_index, self.slide_contents.len() - 1),
            });
        }
    ...
    pub fn replace_text_in_slide(&mut self, slide_index: usize, old: &str, new: &str) -> Result<usize, DocumentError> {
        if slide_index >= self.slide_contents.len() {
            return Err(DocumentError::InvalidStructure {
                reason: format!("Slide index {} out of range (0-{})", slide_index, self.slide_contents.len() - 1),
            });
...
impl DocumentProvider for PowerPointProvider {
    fn replace_text(&mut self, old: &str, new: &str) -> Result<usize, DocumentError> {
        debug!("Replacing text '{}' with '{}' in PowerPoint document", old, new);
        ...
        if total_count > 0 {
            self.modified = true;
            debug!("Replaced {} total occurrences in PowerPoint document", total_count);
        }
...
<p:sld xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main" 
    ...
    <p:cSld>
        <p:spTree>
            ...
            writer.start_file("ppt/slides/slide1.xml", SimpleFileOptions::default()).unwrap();
...
<p:sld xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main" 
    ...
    <p:cSld>
        <p:spTree>
            ...
            writer.start_file("ppt/slides/slide2.xml", SimpleFileOptions::default()).unwrap();
