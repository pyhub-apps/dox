...
use dox_document::{create_provider, DocumentProvider, DocumentType, WordProvider, PowerPointProvider};
use std::io::Write;
...
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
    <w:body>
        ...
        writer.start_file("_rels/.rels", SimpleFileOptions::default()).unwrap();
...
fn test_error_handling() {
    ...
    assert!(matches!(result.unwrap_err(), DocumentError::DocumentNotFound { .. }));
    ...
    assert!(matches!(result.unwrap_err(), DocumentError::UnsupportedFormat { .. }));
}